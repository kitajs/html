...
Rspress supports not only Markdown but also [MDX](https://mdxjs.com/), a powerful way to
develop content.

## Markdown

MDX is a superset of Markdown, which means you can write Markdown files as usual. For
example:
...
You can add Front Matter at the beginning of your Markdown file, which is a YAML-formatted
object that defines some metadata. For example:
...
You can use the `:::` syntax to create custom containers and support custom titles. For
example:
...
If you want to display line numbers, you can enable the `showLineNumbers` option in the
config file:
...
If you want to wrap long code line by default, you can enable the `defaultWrapCode` option
in the config file:
...
:::warning The backslash (`\`) in `[\!code highlight]` is for Markdown escaping to display
the raw syntax. Do not include it when using this notation in your actual code. :::

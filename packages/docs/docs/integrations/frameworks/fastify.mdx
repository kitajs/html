---
description:
  Render JSX with Fastify using reply.html(), stream automatic or explicit Suspense,
  handle async errors, and control automatic doctype injection.
---
...
To avoid passing `req.id` through the component tree, enable automatic Suspense support
...
Rendering `AutoSuspense` directly in a route without `autoSuspense: true` throws an error.
Keep using regular `Suspense` when you prefer an explicit `rid`.

The plugin establishes the automatic request-ID scope in Fastify's `onRequest` hook. This
is intentionally earlier than `preHandler`, so parsing, validation, later user hooks, and
the route handler inherit the same async-local request ID. A scope created in `preHandler`
would not cover asynchronous work started by earlier hooks.

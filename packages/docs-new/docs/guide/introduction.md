...
KitaJS Html is a **super-fast JSX runtime** that generates HTML strings directly, without the overhead of a virtual DOM. It's designed for server-side rendering and provides an elegant, type-safe way to generate HTML.

## What is KitaJS Html?

Unlike React which builds a virtual DOM and then renders it to HTML, KitaJS Html takes JSX syntax and immediately produces HTML strings. This makes it:
...
That's it! No virtual DOM, no reconciliation, no hydration. Just fast, straightforward HTML string generation.
...
Ready to start using KitaJS Html? Head over to the [Getting Started guide](/guide/getting-started) to set up your project.
